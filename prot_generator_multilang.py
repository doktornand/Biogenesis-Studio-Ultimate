#!/usr/bin/env python3
"""
prot_generator_multilang.py — Moteur de generation v3.0
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

NOUVEAUTES :
  • Support multi-langages : Python, C#, JavaScript, Rust, Go
  • Profils de langage extensibles
  • Generation syntaxiquement correcte pour chaque langage cible
"""
...
LANG_PROFILES: Dict[str, Dict[str, Any]] = {
    "python": {
        ...
        "comment": "#",
        "indent": "    ",
        "class_def": "class BioProgram:",
        "init_method": "def __init__(self):",
        "exec_method": "def execute(self):",
        "state_var": "self._cellularState",
        "entropy_var": "self._entropy",
        "organelles": "self._organelles",
        "epigenetic_memory": "self._epigeneticMemory",
        "cell_generation": "self._cellGeneration",
        "telomere_length": "self._telomereLength",
        "stuck_counter": "self._stuckCounter",
        "imports_block": "import random\nimport math\nfrom datetime import datetime\n",
        "random_method": "random.random()",
        "randint_method": "random.randint",
        "console_write": "print",
        "syntax": {
            "if": "if {cond}:",
            "elif": "elif {cond}:",
            "else": "else:",
            "for": "for {var} in range({count}):",
            "while": "while {cond}:",
            "try": "try:",
            "except": "except Exception as e:",
            "inc_state": "self._cellularState += 1",
            "dec_state": "self._cellularState = max(0, self._cellularState - 1)",
        }
    },
    "csharp": {
        "ext": ".cs",
        ...
        "init_method": "public BioProgram()",
        "exec_method": "public void Execute()",
        "state_var": "_cellularState",
        "entropy_var": "_entropy",
        "organelles": "_organelles",
        "epigenetic_memory": "_epigeneticMemory",
        "cell_generation": "_cellGeneration",
        "telomere_length": "_telomereLength",
        "stuck_counter": "_stuckCounter",
        "imports_block": "using System;\nusing System.Collections.Generic;\n",
        "random_method": "new Random().NextDouble()",
        "randint_method": "new Random().Next",
        "console_write": "Console.WriteLine",
        "syntax": {
            "if": "if ({cond})",
            "elif": "else if ({cond})",
            "else": "else",
            "for": "for (int {var} = 0; {var} < {count}; {var}++)",
            "while": "while ({cond})",
            "try": "try",
            "except": "catch (Exception e)",
            "inc_state": "_cellularState++",
            "dec_state": "_cellularState = Math.Max(0, _cellularState - 1)",
        }
    },
    "javascript": {
        "ext": ".js",
        ...
        "exec_method": "execute()",
        "state_var": "this._cellularState",
        "entropy_var": "this._entropy",
        "organelles": "this._organelles",
        "epigenetic_memory": "this._epigeneticMemory",
        "cell_generation": "this._cellGeneration",
        "telomere_length": "this._telomereLength",
        "stuck_counter": "this._stuckCounter",
        "imports_block": "",
        "random_method": "Math.random()",
        ...
        "syntax": {
            ...
            "elif": "else if ({cond})",
            "else": "else",
            "for": "for (let {var} = 0; {var} < {count}; {var}++)",
            "while": "while ({cond})",
            "try": "try {{",
            "except": "}} catch (e) {{",
            "inc_state": "this._cellularState++",
            "dec_state": "this._cellularState = Math.max(0, this._cellularState - 1)",
        }
    },
    "rust": {
        "ext": ".rs",
        "comment": "//",
        "indent": "    ",
        "class_def": "pub struct BioProgram",
        "init_method": "pub fn new() -> Self",
        "exec_method": "pub fn execute(&mut self)",
        "state_var": "self.cellular_state",
        "entropy_var": "self.entropy",
        ...
        "random_method": "rand::thread_rng().gen::<f64>()",
        "randint_method": "rand::thread_rng().gen_range",
        "console_write": "println!",
        "syntax": {
            "if": "if {cond}",
            "elif": "else if {cond}",
            "else": "else",
            "for": "for {var} in 0..{count}",
            "while": "while {cond}",
            "try": "// Rust uses Result type",
            "except": "// Error handling with match",
            "inc_state": "self.cellular_state += 1",
            "dec_state": "self.cellular_state = if self.cellular_state > 0 {{ self.cellular_state - 1 }} else {{ 0 }}",
        }
    },
    "go": {
        "ext": ".go",
        "comment": "//",
        "indent": "\t",
        "class_def": "type BioProgram struct",
        "init_method": "func NewBioProgram() *BioProgram",
        "exec_method": "func (b *BioProgram) Execute()",
        "state_var": "b.cellularState",
        "entropy_var": "b.entropy",
        "organelles": "b.organelles",
        "epigenetic_memory": "b.epigeneticMemory",
        "cell_generation": "b.cellGeneration",
        "telomere_length": "b.telomereLength",
        "stuck_counter": "b.stuckCounter",
        "imports_block": "import (\n    \"fmt\"\n    \"math/rand\"\n    \"time\"\n)\n",
        "random_method": "rand.Float64()",
        "randint_method": "rand.Intn",
        "console_write": "fmt.Println",
        "syntax": {
            "if": "if {cond}",
            "elif": "else if {cond}",
            "else": "else",
            "for": "for {var} := 0; {var} < {count}; {var}++",
            "while": "for {cond}",
            "try": "// Go uses error returns",
            "except": "if err != nil",
            "inc_state": "b.cellularState++",
            "dec_state": "if b.cellularState > 0 {{ b.cellularState-- }}",
        }
    }
}
...
class ProtParser:
    ...
    def _parse_line(self, lines: List[str], i: int) -> Tuple[Optional[ASTNode], int]:
        ...
        if stripped.startswith("{{mutate:"):
            return self._parse_inline_block(lines, i, "{{endmutate}}", NodeType.MUTATE,
                                            lambda s: float(re.search(r"{{mutate:([\d.]+)}}", s).group(1)))
        if stripped.startswith("{{oscillate:"):
            return self._parse_inline_block(lines, i, "{{endoscillate}}", NodeType.OSCILLATE,
                                            lambda s: int(re.search(r"{{oscillate:(\d+)}}", s).group(1)))
        if stripped.startswith("{{quine:}}"):
            return ASTNode(NodeType.QUINE), i + 1
        if stripped.startswith("{{transposon:"):
            tid = re.search(r"{{transposon:(\w+)}}", stripped).group(1)
            return ASTNode(NodeType.TRANSPOSON, value=tid), i + 1
        if stripped.startswith("{{plasmid:"):
            pid = re.search(r"{{plasmid:(\w+)}}", stripped).group(1)
            return ASTNode(NodeType.PLASMID, value=pid), i + 1
        if stripped.startswith("{{operon_check:"):
            oid = re.search(r"{{operon_check:(\w+)}}", stripped).group(1)
            return ASTNode(NodeType.OPERON, value=oid), i + 1
        if stripped.startswith("{{self_ref:"):
            ref = re.search(r"{{self_ref:(\w+)}}", stripped).group(1)
            return ASTNode(NodeType.SELF_REF, value=ref), i + 1
        if stripped.startswith("{{ca_state:"):
            rule = re.search(r"{{ca_state:(\S+)}}", stripped).group(1)
    ...
    def _parse_conditional(self, lines: List[str], start: int) -> Tuple[ASTNode, int]:
        ...
        while i < len(lines):
            ...
            if depth == 0:
                if s.startswith("{{if:"):
                    current_cond = re.search(r"{{if:(.*?)}}", s).group(1)
                elif s.startswith("{{elif:"):
                    if current_cond is not None:
                        sub = ASTNode(NodeType.COND, value=current_cond,
                                      children=self.parse(current_lines))
                        node.children.append(sub)
                    current_cond = re.search(r"{{elif:(.*?)}}", s).group(1)
                    current_lines = []
                elif s == "{{else}}":
                    if current_cond is not None:
                        sub = ASTNode(NodeType.COND, value=current_cond,
    ...
    def _parse_repeat(self, lines: List[str], start: int) -> Tuple[ASTNode, int]:
        count = int(re.search(r"{{repeat:(\d+)}}", lines[start].strip()).group(1))
        block, end = self._extract_until(lines, start + 1, "{{endrepeat}}")
        node = ASTNode(NodeType.REPEAT, meta={"count": count},
    ...
    def _extract_until(self, lines: List[str], start: int,
                        marker: str) -> Tuple[List[str], int]:
        out, i = [], start
...
class ProtRenderer:
    ...
    def render(self, nodes: List[ASTNode], ctx: GenerationContext,
               raw_template: List[str] = None) -> List[str]:
        out = []
        for node in nodes:
            out.extend(self._render_node(node, ctx, raw_template))
        return out

    def _render_node(self, node: ASTNode, ctx: GenerationContext,
                     raw_template: List[str] = None) -> List[str]:
        ...
        if node.ntype == NodeType.RAW:
            ...
            for branch in node.children:
                try:
                    ...
                    if eval(branch.value, {"__builtins__": {}}, ns):
            ...
            if tp and random.random() < tp.get("jump_prob", 0.1):
                ...
                return [f"{indent}// [TRANSPOSON:{tid}] Saut detecte",
                        f"{indent}{effect}"]
            ...
            if sub_def:
                code = self.engine._generate_protein(
                    sub_def, depth=len(ctx.recursion_stack))
    ...
    def _resolve(self, directive: str, ctx: GenerationContext) -> Any:
        ...
        return ctx.variables.get(directive,
               ctx.globals.get(directive, f"/* var:{directive} */"))
...
class ProtEngine:
    def __init__(self, config_path: str, output_dir: str = "./slot",
        ...
        if self.target_lang not in LANG_PROFILES:
            raise UnsupportedLanguageError(
                f"Langage '{target_lang}' non supporte. "
                f"Choisir parmi: {list(LANG_PROFILES.keys())}"
            )
    ...
    def _resolve_inheritance(self, prot_def: Dict) -> Dict:
        ...
        merged.update({k: v for k, v in prot_def.items()
                       if k not in ("extends", "override")})
    ...
    def generate_all(self):
        ...
        print(f"Generation v3 — {len(proteins)} proteines, "
              f"generation {self.generation}, langage {lang_name}...")
    ...
    def _generate_protein(self, prot_def: Dict, depth: int = 0) -> str:
        ...
        self.manifest.append({
            "id": prot_id, "filename": filename,
            "checksum": chk, "generation": self.generation,
            "lines": len(code_lines), "language": self.target_lang,
            "epigenetic_markers": ctx.epigenetic,
            "extends": prot_def.get("extends"),
        })
        return code

    def _generate_class_wrapper(self, ast: List[ASTNode], ctx: GenerationContext,
                                 raw_template: List[str]) -> List[str]:
        profile = ctx.lang_profile
        indent = profile.get("indent", "    ")
        comment = profile.get("comment", "//")
        code = []
        ...
        code.append(f"{comment} GENERATION AUTOMATIQUE — Bio-Compiler v3")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Distillateur de proteines v3.0 — Multi-langages",
    ...
    parser.add_argument("-g", "--generation", type=int, default=0,
                        help="Numero de generation")
    parser.add_argument("-l", "--lang", default="csharp",
                        choices=list(LANG_PROFILES.keys()),
                        help="Langage cible")

    args = parser.parse_args()

    engine = ProtEngine(args.config, args.output,
